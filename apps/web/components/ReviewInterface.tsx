...
interface ReviewInterfaceProps {
    ...
    project: Project;
    comments: Comment[];
    onCreateComment?: (payload: { x: number; y: number; message: string; pageUrl: string }) => Promise<void>;
    onUpdateCommentStatus?: (commentId: string, status: 'OPEN' | 'RESOLVED') => Promise<void>;
...
export default function ReviewInterface({
    ...
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            ...
            if (event.data.type === 'handshake' || event.data.type === 'path-update') {
                if (!isEmbedDetected) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            alert('Failed to submit comment. Please ensure Page Path is valid (starts with /).');
        } finally {
            setSubmitting(false);
        }
    };
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {mode === 'agency' && (
                                <div className="flex items-center gap-2 mr-4 border-r border-white/10 pr-4">
                                    <button
                                        onClick={() => {
                                            const scriptTag = `<script src="${window.location.origin}/embed.js" async></script>`;
                                    ...
                                    <div className="group relative cursor-help">
                                        <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-[var(--text-2)] hover:text-white transition-colors">?</span>
                                        <div className="absolute right-0 top-full mt-2 w-64 p-3 bg-[#1A1A1A] border border-white/10 rounded shadow-xl text-xs z-[60] hidden group-hover:block pointer-events-none">
                                            <div className="font-bold text-white mb-1">Manual Fallback</div>
                                            <p className="text-[var(--text-2)]">
                                                If the client cannot install the script, use <strong>Manual Page Selection</strong> in the top bar to ensuring comments are scoped correctly.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className={styles.previewContainer}>
                    ...
                    {iframeStatus === 'ERROR' && (
                        <div className={styles.fallback}>
                            ...
                            <p className="mb-6 max-w-md">This website blocks embedded previews (X-Frame-Options).</p>
                            <a href={project.baseUrl} target="_blank" className="btn btn-primary">Open Website ↗</a>
                        </div>
                    )}
                        ...
                        >
                            ...
                            {activeCommentId === comment.id && (
                                <div className={styles.pinTooltip}>
                                    ...
                                    <div className="line-clamp-3 overflow-hidden text-ellipsis">{comment.message}</div>
                                </div>
                            )}
                        </div>
                    ))}
                        ...
                        <div ref={overlayRef} className={styles.overlay} onClick={handleOverlayClick}>
                            {!popover.isOpen && (
                                <div className={styles.helperText}>
                                    {prefilledText ? "Click on the page to place your new comment" : "Click anywhere to leave a comment"}
                                    {!isEmbedDetected && (
                            ...
                            {popover.isOpen && (
                                <div ref={popoverRef} className={styles.popover} style={{ left: popover.x, top: popover.y }} onClick={(e) => e.stopPropagation()}>
                                    <textarea
                                        autoFocus
                                    ...
                                    <div className={styles.popoverActions}>
                                        <button className="btn btn-ghost py-1 px-3 text-sm" onClick={(e) => handleCancel(e)}>Cancel</button>
                                        <button className="btn btn-primary py-1 px-3 text-sm" onClick={handleSubmit} disabled={submitting || !commentText.trim()}>
                                            {submitting ? '...' : 'Post'}
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div className={styles.emptyState}>
                            {filterStatus === 'OPEN' ? (
                                <>
                                    ...
                                    <div className={styles.emptyStateTitle}>No pending feedback</div>
                                ...
                                <>
                                    <div className={styles.emptyStateIcon}>✓</div>
                                    <div className={styles.emptyStateTitle}>No resolved items</div>
                                    <p className={styles.emptyStateDesc}>Resolved comments will appear here for reference.</p>
                                </>
                            )}
                        </div>
                    ) : (
                        displayedComments.map((c, i) => (
                            ...
                            >
                                <div className={styles.commentHeader}>
                                    <div className="flex items-center gap-2">
                                        <span className={styles.commentIndex}>
                                            {i + 1}
                                        </span>
                                        ...
                                        {/* PAGE CONTEXT LABEL */}
                                        <span className="text-[10px] text-[var(--muted)] opacity-60 ml-1 truncate max-w-[120px]" title={c.pageUrl}>
                                            {c.pageUrl}
                                        </span>
                                    </div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            {c.status === 'OPEN' && (
                                                <button
                                                    ...
                                                    title="Mark Resolved"
                                                    onClick={(e) => { e.stopPropagation(); onUpdateCommentStatus(c.id, 'RESOLVED'); }}
                                                >
                                                    ✓ Resolve
                                                </button>
                                            )}
                                            {c.status === 'RESOLVED' && (
                                                <button
                                                    ...
                                                    title="Reopen"
                                                    onClick={(e) => { e.stopPropagation(); onUpdateCommentStatus(c.id, 'OPEN'); }}
                                                >
                                                    ↺ Reopen
                                                </button>
                                            )}
                                ...
                                <div className={styles.meta}>
                                    ...
                                    {c.status === 'RESOLVED' && (
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setPrefilledText(`Regarding resolved comment #${i + 1}: `);
                                                setCommentMode(true);
                                                setFilterStatus('OPEN');
                                            }}
                                        >
